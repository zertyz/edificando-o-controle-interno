...
export class MPSubscribeComponent implements OnInit {
  ...
  ngOnInit() {
    if (MPSubscribeComponent.maxModalAutoShow > 0) {
      const modalRef = this.modalService.open(MPSubscribeContentComponent, {size: 'lg'});
      MPSubscribeComponent.maxModalAutoShow--;
    }
  }
